...
f1=readmatrix('./board1/o1v189.csv');
eventsize=640;
filesize=size(f1);
j=0;
...
f1=readmatrix('./board1/o2v189.csv');
j=0;
for i=3:filesize(1,1)
    if(i== ((eventnumb-1000)*eventsize+3))
        eventnumb=eventnumb+1; 
        j=0;
    end
    j=j+1;
...
f1=readmatrix('./board1/o3v189.csv');
j=0;
for i=3:filesize(1,1)
    if(i== ((eventnumb-2000)*eventsize+3))
        eventnumb=eventnumb+1; 
        j=0;
    end
    j=j+1;
...
f1=readmatrix('./board1/o4v189.csv');
j=0;
for i=3:filesize(1,1)
    if(i== ((eventnumb-3000)*eventsize+3))
        eventnumb=eventnumb+1; 
        j=0;
    end
    j=j+1;
...
ylabel('Counts (arb.)');
title('Board1 OR4 ToA distribution with 6mV step input');
legend('Measurement DATA','Fit');
